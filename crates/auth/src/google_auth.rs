...
use common::{errors::AuthErrorTypes, domain_models::auth};
...
pub async fn verify_google_login(
    ...
    let header = decode_header(request.id_token.clone()).change_context(AuthErrorTypes::InvalidGoogleToken)?;
    ...
    validation.set_issuer(&[
        GOOGLE_ISSUER_ENDPOINT,
        GOOGLE_ISSUER_DOMAIN
    ]);

      let token_data = decode::<auth::GoogleUserClaims>(
    ...
    log::info!("user_verified: {}", claims.email.clone().ok_or(AuthErrorTypes::DataNotFound { field_name: "Email".to_string() })?.expose_secret());
